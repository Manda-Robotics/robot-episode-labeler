...
Working notes for the rework. `docs/results.md` is the record of *measured*
claims; this file records what was tried, in order, including what did not work.
Numbers are on WGO-Bench unless stated. Every corpus number is subject to the
±0.03–0.08 F1 noise floor in `docs/results.md`; nothing is claimed without
`scripts/variance.py`.
...
1. **Grid quantisation.** 84% of predicted boundaries sit exactly on the 0.5 s
   sampling grid. ±0.25 s recall (0.22) is capped by the grid.
...
4. **The WGO-comparable metric.** Macrodata's 0.306 is F1 at IoU 0.75 with the
   ...
   because over-splitting is punished at tight IoU, so it is a recall-vs-precision
   trade rather than a free win. Per family (fast): Galaxea 0.61, DROID 0.58,
   HomER 0.24.
...
- **State-then-boundaries.** The strongest zero-shot grounding results (REZE: 51.5
  vs 6.5 mIoU for the same model) come from asking the model to classify frame
  state and deriving boundaries in code, rather than asking for timestamps.
...
problem is under-segmentation; boundary error is a symptom of a missing
boundary rather than a misplaced one. This is why subdivision (a split pass) was
the first pass's largest gain, and it is what the pick/place decomposition
prompt and the per-frame state backend attack directly.
...
all within noise; `f1_wgo` −0.050, CI [−0.092, −0.010], **significant**.
...
0.52 → 0.29; recall on 1–2 s events 0.18 → 0.58) but places their edges on grid
...
merges 0.52 → 0.25, recall on 1–2 s events 0.18 → 0.65, on sub-1 s events 0.00 → 0.19. The prompt change is three
...
and confirms the failure-taxonomy diagnosis (merging, not misplacement).
...
These are a check on the segmentation *mechanism* (does the decomposition prompt
over-split a sim scene? does the model track two arms?), not a second
leaderboard. The sim sets are the closest public analogue to Manda's RoboLab
clips. Caveats are recorded per episode in `metadata.notes` (RoboCerebra
`coffee_table/case1` has a 35-frame count mismatch; AgiBot is limited to 11
episodes because the next archives are ≥4.5 GB; gated AgiBot Beta/Alpha and
RoboInter need an HF account with accepted terms).
